...
import { ref } from 'vue'
import type { GameShelfBridge, RootInput } from '../../api/contracts'

const props = defineProps<{ bridge: GameShelfBridge }>()
const emit = defineEmits<{ saved: [root: RootInput]; close: [] }>()
...
async function submit() {
  ...
  const result = await props.bridge.add_root(input)
...
<template>
  <section class="dialog-card" aria-labelledby="add-root-title">
    <div class="section-heading">
      <h2 id="add-root-title">添加游戏目录</h2>
      <button class="icon-button" type="button" aria-label="关闭" @click="$emit('close')">×</button>
    </div>
    <form @submit.prevent="submit">
      <label>目录路径</label>
      <div class="path-row">
        <input v-model="displayPath" data-test="display-path" placeholder="例如 D:\\Games" />
      ...
      <textarea v-model="exclusionsText" rows="3" placeholder="tools&#10;**/cache" />
      <p v-if="error" class="form-error" role="alert">{{ error }}</p>
      <div class="dialog-actions">
        <button type="button" class="secondary" @click="$emit('close')">取消</button>
        <button type="submit" :disabled="submitting">{{ submitting ? '正在添加…' : '添加并扫描' }}</button>
      </div>
    </form>
  </section>
</template>

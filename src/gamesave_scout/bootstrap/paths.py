"""Resolve every GameSave Scout-owned path from the portable application root."""
...
class AppPaths:
    ...
    def ensure_writable(self) -> None:
        probe = self.data_dir / f".gamesave-scout-write-test-{uuid4()}.tmp"
        try:
            self.app_root.mkdir(parents=True, exist_ok=True)
            for directory in self.required_directories():
                directory.mkdir(parents=True, exist_ok=True)
            with probe.open("xb") as stream:
                stream.write(b"GameSave Scout")
                stream.flush()

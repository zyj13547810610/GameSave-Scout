...
from gamesave_scout.rules.catalog import RuleSnapshot
...
from gamesave_scout.saves.engine_hints import (
    EngineSaveHintProvider,
    load_engine_metadata,
)
from gamesave_scout.saves.ludusavi_index import (
    IndexedName,
    IndexedPathRule,
    InvalidLudusaviIndex,
    LudusaviIndex,
)
from gamesave_scout.saves.ludusavi_matcher import normalize_ludusavi_name
from gamesave_scout.saves.ludusavi_models import ManifestCondition, ManifestLocationRule
    ...
    def collect(self, context: BatchRuleContext) -> BatchRuleCatalog:
        ...
        for location in locations:
            ...
            identity = RuleIdentity(
                ...
                evidence=("GameSave Scout 已记录该存档位置", *location.evidence),
            )
            add(
                _raw_candidate(

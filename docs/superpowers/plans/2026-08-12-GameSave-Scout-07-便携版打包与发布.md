...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1 及后续版本；最后更新：2026-08-25；状态：V0.3.3 后端、前端、PyInstaller 配置、双版本发布链技术改名和正式双版本便携候选均已完成，并通过完整自动门禁、两版冻结 smoke 和独立完整性复核。历史 V0.1 的本机真实窗口、路径与 Windows Sandbox 人工验收结论继续作为兼容性证据。完整发布负载绝对路径达到或超过 260 字符仍不受支持。完整目标 Windows 10/11、Evergreen 断网、安装器失败、特殊路径故障和 SmartScreen 实机矩阵保留为后续可选兼容性复核。
...
### 1.1 V0.3.3 发布身份（发布链与正式候选已完成）

V0.3.3 作为新产品技术身份的第一个发布版本，不复用 V0.3.2 文件名或校验记录。以下发布链与正式本地候选均已完成：
...
2026-08-26 完成的多根目录并发扫描修复、顶部全局导航、固定游戏目录、仅存档卡片回退与删除，以及批量封面已有封面标识统一归入 V0.3.4。V0.3.4 便携包尚未构建；完成源码和文档收口后再生成并记录真实清单、大小与 SHA-256。V0.3.3 不作为正式便携版本保留。
...
两个发布目录都不包含 `data`。首次正常启动时，程序才在 `GameSaveScout.exe` 旁创建 `data`。构建和压缩过程不得把开发机上的数据库、配置、封面、游戏、存档或其他本地用户数据带入发布物。2026-08-25 V0.3.3 实测轻量版目录/ZIP 为 92.08/35.24 MiB，完整离线版为 751.35/326.16 MiB，完整版保留完整 Fixed Runtime。V0.3.2 历史候选实测轻量版目录/ZIP 为 92.07/35.23 MiB，完整离线版为 751.35/326.15 MiB。
...
同日从干净提交 `b8ffb7d34a5945b50e4faaad582f1e29d6748d94` 执行 V0.3.3 正式双版本构建。构建入口通过 Python 1276 项、1 项平台条件跳过，前端 57 个测试文件共 241 项、Ruff、mypy、Vue 类型检查、141 模块生产构建、schema 4 源码 smoke 和两版冻结 smoke；PyInstaller 6.22.1 只构建一次核心，再原子生成六个新名称产物。两份清单均记录 `appVersion: 0.3.3`、`databaseSchemaVersion: 4`、上述完整提交号及 `gitDirty: false`。受控 Fixed Runtime CAB 与 Evergreen Bootstrapper 的 Authenticode 状态均为 `Valid`，签名者为 Microsoft Corporation。

构建后把最终 `dist` 两个目录复制回唯一的 `build/release/independent-verify-*` 受控目录，并用同一发布布局验证器独立复核。结果确认新包名、`GameSaveScout.exe`、规则树、四个初始化结构、README 版本、`fixed`/`evergreen` 模式隔离及无 `data` 均正确；两个 ZIP 独立重算哈希与各自 `.sha256` 一致。完整版目录/ZIP 为 751.35/326.16 MiB，轻量版为 92.08/35.24 MiB。V0.3.3 ZIP SHA-256 为：
...
V0.3.3 本轮完成自动门禁、冻结 smoke 和本机构建产物独立完整性复核，没有在额外干净目标设备上重复历史 V0.1 人工矩阵；下列历史兼容性证据继续保留，未执行场景不记为本轮新通过。
...
| 2026-08-25 | 确认 V0.3.3 发布身份设计：`GameSaveScout.spec` / `GameSaveScout.exe`、`GameSave-Scout-0.3.3-*` 双版本六产物和新发布说明一次收口；旧 V0.3.2 产物与校验记录保持原样，构建不自动上传 Release。 |
| 2026-08-25 | 从干净提交 `b8ffb7d34a5945b50e4faaad582f1e29d6748d94` 完成 V0.3.3 双版本正式构建与独立复核；Python 1276 项通过、1 项跳过，前端 241 项及完整门禁、两版冻结 smoke、格式 2 清单、schema 4、规则树、模式隔离、无 `data`、ZIP 与微软签名均通过。完整版/轻量版目录为 751.35/92.08 MiB，ZIP 为 326.16/35.24 MiB，SHA-256 为 `4458399b2a8d4e0f1a251253af4cb23dc99752815391f57593b239fad5444c4b` / `e29f26fd9e0e86c093555f61ab63747abdf857298d0d70437096af812f0fdeb1`。 |

...
**目标：** 通过七个可独立评审、可实际运行的增量，交付已确认的 GameShelf V1 Windows 10/11 x64 便携应用。
...
**技术栈：** Python 3.12、pywebview 6.2.x/WebView2、SQLite、Vue 3、TypeScript、Vite、Vitest、pytest、Pillow、watchdog、psutil、pefile、PyYAML、RapidFuzz、PyInstaller 6.21.x、Node.js 24 LTS。

## 全局约束

- V1 仅面向 Windows 10 和 Windows 11 x64；只有被测单元与平台无关时，开发测试才可在其他主机上运行。
...
- 扫描期间绝不执行 EXE；只有用户明确执行启动操作或存档检测会话时，才运行用户选择的主程序。
- 绝不删除、移动、解包或修改用户的游戏文件和存档文件。
- 后续扫描必须保留用户手动设置的标题、可执行文件、引擎和存档路径。
- 将引擎识别视为可选元数据：引擎未知的 Windows 游戏也必须完整可用。
- 关联孤立存档或接受动态检测到的存档位置前，必须由用户确认。
- V1 不实现翻译、资源提取、DLL 注入、作弊以及存档备份/恢复。
...
已确认的规格涵盖七个故障模式各不相同的子系统。每份链接计划都会产出一个可执行增量，使评审者能够独立验收，而不会被后续工作的故障干扰。

| 增量 | 计划 | 可运行交付物 |
|---|---|---|
| 1 | [基础设施与桌面外壳](2026-08-12-gameshelf-01-foundation.md) | 一个空的便携版 GameShelf 窗口，具备已迁移的 SQLite 数据库和类型化桥接层 |
| 2 | [游戏库扫描与启动](2026-08-12-gameshelf-02-library-scan-launch.md) | 将多个根目录扫描到持久化游戏库中，并可启动游戏和打开文件夹 |
| 3 | [封面与游戏库体验](2026-08-12-gameshelf-03-covers-library-ui.md) | 封面网格、详情抽屉、搜索/筛选、本地选择及剪贴板粘贴 |
| 4 | [引擎识别](2026-08-12-gameshelf-04-engine-recognition.md) | 已确认的正式和实验性引擎识别器，包含证据展示与手动覆盖 |
| 5 | [静态存档位置](2026-08-12-gameshelf-05-static-save-locations.md) | 多个存档路径、便携路径模板、Ludusavi 规则及引擎存档提示 |
| 6 | [动态及孤立存档发现](2026-08-12-gameshelf-06-dynamic-orphan-saves.md) | 引导式监控、候选项评分、扩展扫描及孤立存档审核 |
| 7 | [便携打包与发布](2026-08-12-gameshelf-07-portable-release.md) | 可复现的 onedir 安装包，包含固定版 WebView2 运行时及干净机器检查 |
...
计划 02 完成后，计划 03 和 04 可以任选顺序开发，但两者都必须在计划 05 验收前完成。其他依赖均按顺序执行。
...
| 已确认的设计内容 | 实施计划 |
|---|---|
| 目标、V1 范围、离线/只读/用户确认原则 | 所有计划的全局约束；计划 01、02、05、06 |
...
| Ludusavi 内置规则/手动更新/自定义清单 | 计划 05 |
| 引导式动态检测、溢出降级、进程生命周期、定向注册表监控 | 计划 06 |
| 已删除游戏/孤立存档扫描、仅存档卡片、关联/忽略审核 | 计划 06 |
...
| 单元/集成/虚拟机测试及 V1 验收条件 | 每份计划；最终证据由计划 07 提供 |
| onedir x64、固定版 WebView2、第三方声明、本地产物 | 计划 07 |
| 排除存档备份/恢复及其他第二阶段候选功能 | 全局约束；V1 没有任务实现这些功能 |

...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1 首个可分发版本；最后更新：2026-08-16；状态：轻量版手动安装引导已实现，新双版本候选包已重建并通过本机自动复核；全新 Windows Sandbox 的轻量版手动安装主闭环已通过，其他故障场景及完整 Windows 10/11 人工验收仍待完成。
...
- 对已经重建的候选继续完成 Sandbox 取消、Bootstrapper 缺失/篡改、完整版断网启动等故障与离线场景，以及完整 Windows 10/11 人工验收；此前含静默安装逻辑的候选只保留为历史证据。
...
用户随后在全新 Windows Sandbox 按发布交接步骤验证轻量版主闭环：缺失 Evergreen 时显示新的手动安装提示；选择“是”后 Explorer 打开 `prerequisites` 并选中 Bootstrapper，GameShelf 正常退出，没有自动启动安装器且没有生成 `startup-error.log`；用户手动双击安装器并完成安装后，再次手动启动 GameShelf，主窗口正常出现。该结果证明本次缺陷的主路径已闭环，但不替代取消、安装器缺失/篡改、完整版断网启动以及完整 Windows 10/11 场景矩阵。

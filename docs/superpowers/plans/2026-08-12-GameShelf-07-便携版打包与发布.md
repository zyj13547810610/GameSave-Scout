...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1 首个可分发版本；最后更新：2026-08-16；状态：新双版本候选包已通过本机自动复核；Windows Sandbox 的手动安装主闭环、取消、Bootstrapper 缺失/篡改和完整版无系统 WebView2 断网启动均已通过，完整 Windows 10/11、路径与 SmartScreen 人工验收仍待完成。
...
2026-08-16 已在 Windows Sandbox 完成：轻量版手动安装与重新启动主闭环、用户取消、Bootstrapper 缺失、Bootstrapper SHA-256 不匹配，以及完整版在无系统 WebView2且断网时启动。以下条目仍作为完整首发矩阵，其中已覆盖的场景还需在目标 Windows 10/11 版本和规定路径条件下复核：

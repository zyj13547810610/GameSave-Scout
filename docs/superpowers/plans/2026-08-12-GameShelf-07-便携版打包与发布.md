...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1.x；最后更新：2026-08-18；状态：V0.1.4 源码与发布元数据已经收口，准备使用既有双版本流水线重建包含 SQLite schema 2、批量封面、标题/版本分离和游戏库增量扫描的最新便携候选包。历史 V0.1.0 候选已通过完整自动门禁、本机复核、冻结副本复验以及 Windows Sandbox 的手动安装主闭环和关键故障路径。本机中文、日文、空格和常规长路径已通过，完整发布负载绝对路径达到或超过 260 字符明确不受 V0.1 支持。完整目标 Windows 10/11、Evergreen 断网、安装器失败、特殊路径故障和 SmartScreen 实机矩阵转为后续可选兼容性复核，不再阻塞 V0.1。

## 1. 发布目标与已确认边界

V0.1 产出两个基于同一份 PyInstaller onedir 核心的 Windows 10/11 x64 便携包：

- `GameShelf-0.1.4-win-x64` 是完整离线版，自带 Microsoft Fixed Version WebView2 Runtime x64，可在没有系统 WebView2、没有网络的电脑上运行。
- `GameShelf-0.1.4-win-x64-lite` 是轻量联网版，优先使用系统 Evergreen WebView2 Runtime；系统缺失时，经用户明确同意后打开 `prerequisites` 文件夹并选中微软官方 Bootstrapper，GameShelf 随即正常退出。用户手动双击安装，完成后重新启动 GameShelf。
...
首版不制作 onefile、GameShelf 安装器、自动更新器和精简开发包，不上传产物，也不自动创建公开 Release。轻量版中的 WebView2 Bootstrapper 只是缺失系统运行时的受控先决条件安装器，不改变 GameShelf 本身的便携形态。发布包携带顺序数据库迁移；当前 schema 2 只为标题与版本号增加可空字段，升级前自动备份既有数据库且不重写 schema 1 旧标题。开发阶段仍允许用户自行删除可舍弃数据库并重新扫描，但构建和启动不能依赖这一人工步骤。
...
- 冻结载荷必须同时包含 `_internal/gameshelf/db/migrations/0001_initial.sql` 和 `_internal/gameshelf/db/migrations/0002_initial.sql`；发布关键文件校验缺少任一迁移都必须失败。
...
```json
{
  ...
  "frozen": true,
  "runtimeMode": "fixed",
  "executable": "D:\\Temp\\GameShelf-0.1.4-win-x64\\GameShelf.exe",
  "resourceRoot": "D:\\Temp\\GameShelf-0.1.4-win-x64\\_internal\\resources",
  "webviewRuntime": "D:\\Temp\\GameShelf-0.1.4-win-x64\\runtime",
  "checks": {
    "resources": true,
    "ui": true,
    "engineRules": true,
...
| 2026-08-18 | 源码与后续冻结载荷升级为数据库 schema 2；PyInstaller 和发布关键文件校验同时要求 `0001_initial.sql`、`0002_initial.sql`，源码冒烟通过 schema 2。本轮按计划不重建便携候选包。 |

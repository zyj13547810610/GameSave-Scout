...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1 首个可分发版本；最后更新：2026-08-16；状态：新双版本候选包已通过本机自动复核；Windows Sandbox 的手动安装主闭环、取消、Bootstrapper 缺失/篡改和完整版无系统 WebView2 断网启动均已通过；本机中文、日文、空格和常规长路径已通过，V0.1 明确不支持完整发布负载绝对路径达到或超过 260 字符；完整 Windows 10/11 目标设备验收因当前没有可用设备而暂缓，安装失败与 SmartScreen 矩阵仍待完成。
...
- 对已经重建的候选继续完成轻量版已有 Evergreen 断网、微软安装器断网/失败、完整 Windows 10/11 和 SmartScreen 人工验收；中文/日文/空格及发布负载绝对路径低于 260 字符的常规长路径已经通过本机验收，达到或超过 260 字符明确不属于 V0.1 支持范围，此前含静默安装逻辑的候选只保留为历史证据。
...
Fixed Runtime 主程序与轻量版 Bootstrapper 的 Authenticode 状态均为 `Valid`，签名者为 Microsoft Corporation。在隔离轻量版冻结副本中损坏 `engines.yaml` 后，JSON smoke 仍退出 0，应用构建和引擎规则检查均通过，普通日志记录仅启用内置检测器的 WARNING，且不生成 `startup-error.log`；恢复规则后 smoke 再次通过。该结果确认当前冻结候选包含模块 04 的损坏规则降级修复，但仍不替代完整目标 Windows 10/11、微软安装器失败与 SmartScreen 人工验收。
...
- 中文、日文、空格路径，以及完整发布负载绝对路径低于 260 字符的常规长路径；本机两版已于 2026-08-16 通过。V0.1 明确不支持完整发布负载绝对路径达到或超过 260 字符。
- 跨盘复制按用户决定不执行，本轮未测试且不计入已通过项。

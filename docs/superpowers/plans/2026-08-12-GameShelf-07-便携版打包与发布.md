...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1 及后续版本；最后更新：2026-08-23；状态：V0.2.1 源码、发布元数据和本地双版本便携候选包已完成收口；最新候选包含 SQLite schema 4、自定义分组与批量存档发现。V0.3.1 规则目录、冻结资源 spec、smoke 与发布清单校验已实现，但尚未构建 V0.3.1 便携包。V0.2.1 已通过完整自动门禁、两版冻结 smoke 和正式产物独立完整性复核；历史 V0.1 的本机真实窗口、路径与 Windows Sandbox 人工验收结论继续作为兼容性证据。完整发布负载绝对路径达到或超过 260 字符仍不受支持。完整目标 Windows 10/11、Evergreen 断网、安装器失败、特殊路径故障和 SmartScreen 实机矩阵保留为后续可选兼容性复核。

## 1. 发布目标与已确认边界

当前版本产出两个基于同一份 PyInstaller onedir 核心的 Windows 10/11 x64 便携包：

- `GameShelf-0.2.1-win-x64` 是完整离线版，自带 Microsoft Fixed Version WebView2 Runtime x64，可在没有系统 WebView2、没有网络的电脑上运行。
- `GameShelf-0.2.1-win-x64-lite` 是轻量联网版，优先使用系统 Evergreen WebView2 Runtime；系统缺失时，经用户明确同意后打开 `prerequisites` 文件夹并选中微软官方 Bootstrapper，GameShelf 随即正常退出。用户手动双击安装，完成后重新启动 GameShelf。
...
当前仍不制作 onefile、GameShelf 安装器、自动更新器和精简开发包，构建脚本不上传产物，也不自动创建公开 Release。轻量版中的 WebView2 Bootstrapper 只是缺失系统运行时的受控先决条件安装器，不改变 GameShelf 本身的便携形态。V0.2.1 发布包携带 `0001`～`0004` 四个新库初始化结构并使用 schema 4；开发阶段不提供 schema 1/2/3 到 schema 4 的迁移，检测到旧库时在写入前拒绝启动并提示用户自行移走或删除数据库。
...
两个发布目录都不包含 `data`。首次正常启动时，程序才在 `GameShelf.exe` 旁创建 `data`。构建和压缩过程不得把开发机上的数据库、配置、封面、游戏、存档或其他本地用户数据带入发布物。2026-08-20 V0.2.1 实测轻量版目录/ZIP 为 91.87/35.11 MiB，完整离线版为 751.14/326.03 MiB；完整版保留完整 Fixed Runtime，不以不受支持的删文件方式换取体积下降。
...
```json
{
  ...
  "frozen": true,
  "runtimeMode": "fixed",
  "executable": "D:\\Temp\\GameShelf-0.2.1-win-x64\\GameShelf.exe",
  "resourceRoot": "D:\\Temp\\GameShelf-0.2.1-win-x64\\_internal\\resources",
  "webviewRuntime": "D:\\Temp\\GameShelf-0.2.1-win-x64\\runtime",
  "checks": {
    "resources": true,
    "ui": true,
    "engineRules": true,
...
V0.3.1 发布工具与冻结 smoke 已要求确认内置引擎/存档规则、两个 schema、Ludusavi YAML/元数据/SQLite/许可证均来自 `_internal/resources/rules`，随包快照可以完成只读状态与冷查询检查，首次启动只在临时发布副本旁生成空的用户规则目录，不复制开发机规则或旧 manifest 目录；实际 V0.3.1 冻结包尚未构建，因此不把冻结 smoke 记为已执行。
...
V0.2.1 本轮完成自动门禁、冻结 smoke 和本机构建产物独立完整性复核，没有在额外干净目标设备上重复历史 V0.1 人工矩阵；下列历史兼容性证据继续保留，未执行场景不记为本轮新通过。
...
| 2026-08-23 | 完成 V0.3.1 发布前源码校验：PyInstaller spec 与严格发布布局要求统一规则树，smoke 检查规则目录/许可证/已编译快照，发布元数据读取双规则版本并对 Ludusavi 索引执行冷查询；V0.3.1 便携包仍未构建。 |

...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1 首个可分发版本；最后更新：2026-08-16；状态：V0.1 当前范围已完成。双版本候选包已通过完整自动门禁、本机复核、冻结副本复验以及 Windows Sandbox 的手动安装主闭环和关键故障路径；本机中文、日文、空格和常规长路径已通过，完整发布负载绝对路径达到或超过 260 字符明确不受 V0.1 支持。完整目标 Windows 10/11、Evergreen 断网、安装器失败、特殊路径故障和 SmartScreen 实机矩阵转为后续可选兼容性复核，不再阻塞 V0.1。

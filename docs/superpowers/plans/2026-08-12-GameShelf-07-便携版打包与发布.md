...
> 文档性质：固定模块设计与发布基准；适用版本：V0.1.x；最后更新：2026-08-18；状态：V0.1.4 源码、发布元数据和本地双版本便携候选包已完成收口；最新候选包含 SQLite schema 2、批量封面、标题/版本分离和游戏库增量扫描。V0.1.4 已通过完整自动门禁、两版冻结 smoke 和正式产物独立完整性复核；历史 V0.1.0 的本机真实窗口、路径与 Windows Sandbox 人工验收结论继续作为已完成的兼容性证据。完整发布负载绝对路径达到或超过 260 字符明确不受 V0.1 支持。完整目标 Windows 10/11、Evergreen 断网、安装器失败、特殊路径故障和 SmartScreen 实机矩阵保留为后续可选兼容性复核，不阻塞 V0.1。

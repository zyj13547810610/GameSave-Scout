...
V0.3.x 冻结 smoke 还必须确认内置引擎/存档规则、两个 schema、Ludusavi YAML/元数据/SQLite/许可证均来自 `_internal/resources/rules`，随包快照可以完成只读状态与冷查询检查，首次启动只在临时发布副本旁生成空的用户规则目录，不复制开发机规则或旧 manifest 目录。

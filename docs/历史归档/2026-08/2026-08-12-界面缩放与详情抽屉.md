# GameSave Scout 界面缩放与详情侧栏 Implementation Plan

> 实施状态：已完成。界面缩放、详情模态层、独立滚动、焦点恢复与生产资源均已落地并完成回归验证。
...
- [x] **Step 3: 实现严格校验、读取、应用和保存**

```ts
export const UI_SCALE_OPTIONS = [0.8, 0.9, 1, 1.1, 1.2] as const
export type UiScale = (typeof UI_SCALE_OPTIONS)[number]
export const UI_SCALE_STORAGE_KEY = 'gamesave_scout.ui-scale'
...
- [x] **Step 3: 运行两个测试文件并确认组件不存在或界面尚未集成**
...
- [x] **Step 3: 运行详情测试并确认遮罩、滚动锁和单一按钮断言失败**
...
- [x] **Step 5: 实现详情覆盖层、Escape 监听和滚动锁**

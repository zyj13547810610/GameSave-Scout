# GameSave Scout WebView2 手动安装引导实施计划
...
**Goal:** 将轻量联网版从“GameSave Scout 静默运行并同步等待 WebView2 Bootstrapper”改为“验证安装器后打开其所在文件夹并正常退出，由用户手动安装并重新启动 GameSave Scout”。

**Architecture:** 保留现有 Evergreen 官方 API 检测、发布清单和 Bootstrapper 完整性校验，把 `EvergreenRuntimeInstaller` 收敛为不执行外部安装器的 `EvergreenRuntimeGuide`。缺失 Runtime 时先验证安装器，再通过原生确认框征得同意；确认后仅调用 Windows Shell 打开 Explorer 并选中文件，以专用正常控制流结束本次启动。GameSave Scout 不跟踪微软安装器状态，用户完成安装后重新启动，下一次检测成功才创建主窗口。
...
- GameSave Scout 不得直接执行 `MicrosoftEdgeWebview2Setup.exe`，不得传递 `/silent /install`，不得等待或轮询微软安装器，也不得自动启动第二个 GameSave Scout 进程。
...
- `src/gamesave_scout/bootstrap/webview_bootstrapper.py`：检测 Evergreen、验证 Bootstrapper、提示后打开安装位置；不再执行安装器。
- `src/gamesave_scout/bootstrap/webview_runtime.py`：把 `evergreen` 模式委托给手动引导组件，向上暴露 `allow_manual_guide` 语义。
- `src/gamesave_scout/platform/windows/startup_reporter.py`：提供准确的中文手动安装确认文案。
- `src/gamesave_scout/app.py`：区分“用户取消/已打开安装位置”的正常退出和真实启动故障。
...
- `docs/superpowers/plans/2026-08-12-GameSave-Scout-07-便携版打包与发布.md`、`docs/superpowers/plans/2026-08-12-GameSave-Scout-开发路线图.md`：设计已经批准；只有真实重建和 Sandbox 复验取得新证据后才更新完成状态、体积和哈希。
...
**Files:**
- Modify: `src/gamesave_scout/bootstrap/webview_bootstrapper.py:1-180`
...
在 `src/gamesave_scout/bootstrap/webview_bootstrapper.py` 删除 `time`、`Sequence`、`CompletedProcess`、`CommandRunner`、`Clock`、`Sleeper`、`_require_success()`、`_run_bootstrapper()`、`_wait_for_runtime()` 和 `timeout_seconds`，保留检测、文件验证和 SHA-256。引入 `stat`，并让 `_validate_bootstrapper()` 在 `path.is_file()` 之外调用 `_is_reparse_point(path)`，拒绝符号链接、目录联接和其他重解析点。实现：
...
def test_location_opener_selects_bootstrapper_with_system_explorer(
    ...
    bootstrapper = tmp_path / "GameSave Scout" / "prerequisites" / "MicrosoftEdgeWebview2Setup.exe"
...
.\.venv\python.exe -m ruff check src/gamesave_scout/bootstrap/webview_bootstrapper.py tests/unit/bootstrap/test_webview_bootstrapper.py
.\.venv\python.exe -m mypy src/gamesave_scout/bootstrap/webview_bootstrapper.py
git diff --check
```
...
**Files:**
- Modify: `src/gamesave_scout/bootstrap/webview_runtime.py:1-207`
- Modify: `src/gamesave_scout/app.py:1-229`
...
在 `src/gamesave_scout/bootstrap/webview_runtime.py` 使用：
...
.\.venv\python.exe -m ruff check src/gamesave_scout/bootstrap/webview_runtime.py src/gamesave_scout/app.py tests/unit/bootstrap/test_webview_runtime.py tests/unit/test_app_startup.py
.\.venv\python.exe -m mypy src/gamesave_scout/bootstrap/webview_runtime.py src/gamesave_scout/app.py
git diff --check
git status --short
```
...
**Files:**
- Modify: `src/gamesave_scout/platform/windows/startup_reporter.py:1-78`
- Modify: `tests/unit/platform/windows/test_startup_reporter.py:1-76`
- Modify: `release/README-lite.txt:1-46`
- Verify: `docs/superpowers/plans/2026-08-12-GameSave-Scout-07-便携版打包与发布.md`
- Verify: `docs/superpowers/plans/2026-08-12-GameSave-Scout-开发路线图.md`
...
result = self.message_box(
    ...
    "GameSave Scout 将打开安装器所在文件夹。\n"
    "请双击 MicrosoftEdgeWebview2Setup.exe 联网完成安装，\n"
    "安装完成后重新启动 GameSave Scout。\n\n"
    "是否打开安装位置？",
...
```text
系统缺失 Evergreen 时，GameSave Scout 会先校验随包的微软官方安装器，并询问是否
...
Select-String -LiteralPath `
  'src/gamesave_scout/bootstrap/webview_bootstrapper.py', `
  'src/gamesave_scout/bootstrap/webview_runtime.py', `
...
- Modify after evidence: `docs/superpowers/plans/2026-08-12-GameSave-Scout-07-便携版打包与发布.md`
- Modify after evidence: `docs/superpowers/plans/2026-08-12-GameSave-Scout-开发路线图.md`
...
5. 在任务管理器确认 `GameShelf.exe` 已退出，而且 GameSave Scout 没有自动创建 `MicrosoftEdgeWebview2Setup` 进程。
6. 确认不存在 `data\logs\startup-error.log`。

Expected: GameSave Scout 不再等待外部安装器。

- [x] **Step 4: 完成手动安装和重新启动闭环**

1. 用户在 Explorer 中双击已选中的 `MicrosoftEdgeWebview2Setup.exe`。
2. 微软安装器联网完成安装；GameSave Scout 不参与安装进度或错误处理。
...
- 轻量版选择“否”：GameSave Scout 退出，Explorer 不打开，不写日志。
...
Expected: 全部命令退出码为 `0`，固定文档无占位符；不得执行暂存、提交或推送，等待用户明确决定。
...
- 缺失 Runtime 时先验证受控文件，再由用户确认打开 Explorer；GameSave Scout 正常退出。
...
- 全新 Windows Sandbox 完成“提示—打开目录—GameSave Scout 退出—手动安装—重新启动”闭环。

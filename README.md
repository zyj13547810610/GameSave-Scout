...
> 当前 V0.3.6 源码与发布链统一使用 **GameSave Scout**：Python 包为 `gamesave_scout`，命令为 `gamesave-scout`，可执行文件为 `GameSaveScout.exe`，发布前缀为 `GameSave-Scout`。程序旁 `data` 和 SQLite schema 4 保持不变；包含窗口尺寸持久化的完整离线版与轻量联网版已从干净提交重建并完成本地复核。
...
- 使用封面网格搜索和筛选游戏，通过主页“批量选择”一次处理已安装或失效记录，并支持 80%～120% 五档界面缩放；正常关闭后会在下次启动恢复最后的普通窗口尺寸；
- 创建、重命名和删除扁平自定义分组；一个游戏可属于多个分组，并支持详情编辑、批量加入/移出以及与搜索、状态、引擎组合筛选；
- 为单个游戏选择、粘贴、替换或移除封面，也可在批量封面工作台中直接启动当前游戏，并从 VNDB、拖放/粘贴、1～3 层游戏目录扫描和自定义非递归封面目录收集候选；导入目录中的 hash、UUID 和截图编号图片进入会话级共享池，文件名只影响排序；
...
- 提供用户主动启动的批量存档发现工作台：在受限范围内扫描并审核已安装、本体失效、未关联及已记录位置，可关联现有游戏或创建仅存档卡片；创建后可从任一关联候选撤销整张卡片，也可在详情删除，实际存档始终保留；
...
- 桌面窗口默认为 `1180 × 760`、最小为 `960 × 640`；正常关闭时把最大化前的普通还原尺寸保存到配置 v6，下次启动按当前主屏工作区限制；不保存窗口位置和最大化状态；
- 当前自动门禁为 Python 1386 项通过、1 项平台条件跳过，前端 58 个测试文件共 274 项通过，并通过 Ruff、mypy（132 个源码文件）、Vue 类型检查、141 模块生产构建和隔离 schema 4 源码 smoke。真实窗口已确认普通关闭恢复尺寸正确，最大化关闭后恢复最大化前的普通尺寸且不会自动最大化。
...
- 后端、前端、发布链、文档和历史代码原型均完成新身份收口；
- V0.3.3 只保留源码里程碑记录，不作为正式便携版本；最近一次保留的历史便携候选为 V0.3.2。
...
V0.1.x、V0.2.0、V0.2.1 和 V0.3.2 已完成既定源码与本地候选收口。当前 V0.3.6 在 V0.3.5 引擎生态分类和第三批规则基础上，完成启动工作目录派生、封面自动优化、详情/批量设置同步、批量封面直接启动、1～3 层图片扫描、导入目录共享候选池和桌面窗口尺寸持久化；配置版本统一为 6，schema 保持 4，完整自动门禁、窗口尺寸人工验收和双版本便携候选复核均已完成。缺少样本的真实游戏规则矩阵仍保留为后续工作。完整目标 Windows 10/11 设备、SmartScreen、UNC/只读目录和特殊运行时故障矩阵没有全部执行，作为后续可选兼容性复核。

V0.2 批量存档发现展示已安装、失效、未关联及已记录存档位置，而不只显示“孤立”结果；候选始终由用户审核，不自动确认归属。存档备份、恢复、同步和版本管理继续后置，目前尚未实现。

## 规则管理

从窗口顶部选择“规则管理”后，可以在三个标签间切换：

- “引擎规则”：查看内置规则，或新建用户引擎识别规则；新建时需要选择“通用 / 主流游戏”或“视觉小说 / 同人游戏”生态，旧的未分类用户规则仍可编辑；保存后在下一次“重新检测”或完整扫描时生效。

...
> 当前 V0.3.6 源码与发布链统一使用 **GameSave Scout**：Python 包为 `gamesave_scout`，命令为 `gamesave-scout`，可执行文件为 `GameSaveScout.exe`，发布前缀为 `GameSave-Scout`。程序旁 `data` 和 SQLite schema 4 保持不变；新增窗口尺寸持久化后，V0.3.6 便携包需重新构建。
...
V0.1.x、V0.2.0、V0.2.1 和 V0.3.2 已完成既定源码与本地候选收口。当前 V0.3.6 在 V0.3.5 引擎生态分类和第三批规则基础上，完成启动工作目录派生、封面自动优化、详情/批量设置同步、批量封面直接启动、1～3 层图片扫描、导入目录共享候选池和桌面窗口尺寸持久化；配置版本统一为 6，schema 保持 4，完整自动门禁与窗口尺寸人工验收均已完成，便携包待本轮重建。缺少样本的真实游戏规则矩阵仍保留为后续工作。完整目标 Windows 10/11 设备、SmartScreen、UNC/只读目录和特殊运行时故障矩阵没有全部执行，作为后续可选兼容性复核。

...
当前源码尚未实现已完成设计的“批量存档发现”工作台，也不包含存档备份、恢复、同步、云服务和常驻后台监控。
...
V0.1.x 已完成源码、便携包与既定验收收口。V0.2.0 自定义游戏分组的源码实现和当前可操作范围的真实 pywebview 人工验收均已完成；尚无用户创建入口的 `save_only` 分组交互保留自动回归证据，并顺延到批量存档发现完成后的真实联调。完整目标 Windows 10/11 设备、SmartScreen、UNC/只读目录和特殊运行时故障矩阵没有全部执行，不记为已通过，作为后续可选兼容性复核。

V0.2 批量存档发现与可能游戏反推的设计已经确认，尚待编写实施计划和实现；它将展示已安装、失效、未知及已记录存档位置，而不只显示“孤立”结果。V0.3 或更晚再考虑存档备份、恢复、同步和版本管理。这些后续功能目前尚未实现。
